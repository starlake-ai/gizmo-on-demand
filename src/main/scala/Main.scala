...
object Main extends LazyLogging:
  ...
  /** Validate API key */
  private def validateApiKey(providedKey: Option[String]): Either[ErrorResponse, Unit] =
  ...
  def main(args: Array[String]): Unit =
    ...
    EnvVars.apiKey match
      ...
      case None => logger.warn("API key authentication is DISABLED - set SL_GIZMO_API_KEY to enable")
    ...
    val startProcessEndpoint = ProcessEndpoints.startProcess.handle { case (apiKey, request) =>
    ...
    val stopProcessEndpoint = ProcessEndpoints.stopProcess.handle { case (apiKey, request) =>
      validateApiKey(apiKey) match
        case Left(error) => Left(error)
        case Right(_) =>
    ...
    val restartProcessEndpoint = ProcessEndpoints.restartProcess.handle { case (apiKey, request) =>
      validateApiKey(apiKey) match
        case Left(error) => Left(error)
        case Right(_) =>
    ...
    val listProcessesEndpoint = ProcessEndpoints.listProcesses.handle { apiKey =>
      validateApiKey(apiKey) match
        case Left(error) => Left(error)
